...
kind-cluster: ${KIND}
	if [[ -z "$$OIDC_S3_BUCKET_NAME" ]]; then \
		...
		cat e2e/kind_config/config.yaml | sed "s/{{S3_BUCKET_NAME}}/$$OIDC_S3_BUCKET_NAME/g" \
		> /tmp/config.yaml
	fi
...
.PHONY: test-no-irsa
test-no-irsa: cluster e2etest

.PHONY: test-with-irsa
test-with-irsa: cluster install-eks-webhook e2etest
